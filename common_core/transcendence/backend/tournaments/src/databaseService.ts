...
export class DatabaseService {
	...
	createTournament(tournament: any): Promise<any> {
		return new Promise((resolve, reject) => {
			const { name, userId, players, settings } = tournament;
			const id = 'tourn-' + Date.now().toString();
			this.db.run(
				`INSERT INTO tournaments (id, name, creator_id, players, max_players, status, settings)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
				[
					id,
					name,
	...
	getUserTournaments(userId: string): Promise<any[]> {
		return new Promise((resolve, reject) => {
			this.db.all(
				`SELECT * FROM tournaments WHERE creator_id = ?`,
				[userId],
	...
	getTournamentById(tournamentId: string): Promise<any | null> {
		return new Promise((resolve, reject) => {
			this.db.get(
				...
				(err: Error | null, row: any) => {
					if (err) reject(err);
						...
						if (tournament) {
							...
							tournament.matches = JSON.parse(tournament.matches);
	...
	updateTournamentScores(tournamentId: string, scores: string[]): Promise<any> {
		const winnerId = determineTournamentWinner(scores);
		const status = scores.length === 4 ? "completed" : "in-progress";

		return new Promise((resolve, reject) => {
			this.db.run(
				`UPDATE tournaments
                SET matches = ?, winner_id = ?, status = ?
                WHERE id = ?`,
				[JSON.stringify(scores), winnerId, status, tournamentId],
				(err: Error | null, row: any) => {
					if (err) reject(err);
					else resolve(row);
				}
...
function determineTournamentWinner(scores: Array<string>): number | null {
	if (scores.length != 4) return null;
	let winCounts: Array<number> = Array(4).fill(0);

	for (let score_index = 0; score_index < scores.length; score_index++) {
		const [score1, score2] = scores[score_index].split(":").map(s => parseInt(s, 10));
		if (score_index == 0)
			score1 > score2 ? winCounts[0]++ : winCounts[1]++;
		else if (score_index == 1)
			score1 > score2 ? winCounts[2]++ : winCounts[3]++;
		else if (score_index == 2)
			continue;
		else if (score_index == 3)
			return score1 > score2 ? winCounts.indexOf(1, 0) : winCounts.indexOf(1, 2);
		else return null;
	}
	return null;
}
